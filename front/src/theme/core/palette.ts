...
// Use defensive local utils to avoid runtime crashes if external helpers fail
import { opacity } from './opacity';
import { themeConfig } from '../theme-config';
import {
  varAlphaSafe as varAlpha,
  createPaletteChannelSafe as createPaletteChannel,
} from '../utils/color';
...
// Ensure numeric tones exist in production builds that call augmentColor('500')
// without having to rely on external helpers or environment quirks.
const ensureNumericTones = (c: Record<string, unknown>) => {
  if (!c) return c;
  if (!('500' in c) && c.main) c['500'] = c.main;
  if (!('200' in c) && c.light) c['200'] = c.light;
  if (!('700' in c) && c.dark) c['700'] = c.dark;
  return c;
};

ensureNumericTones(primary);
ensureNumericTones(secondary);
ensureNumericTones(info);
ensureNumericTones(success);
ensureNumericTones(warning);
ensureNumericTones(error);
...
export const extendPalette: PaletteExtend = {
  shared: {
    inputUnderline: varAlpha(grey['500Channel'], Number(opacity.inputUnderline ?? 0.32)),
