...
export function useTranslate(namespace?: Namespace) {
  ...
  const handleChangeLang = useCallback(
    async (lang: LangCode) => {
      try {
        ...
        if (authenticated && user) {
          try {
            await updateProfile({ user_language: lang });
          } catch (error) {
            // Log error for debugging
            console.error('Failed to save language preference to profile:', error);
            // Don't show error toast - language change still succeeded
          }
        }
      } catch (error) {
        console.error(error);
    ...
    [i18n, tMessages, updateDayjsLocale, updateDirection, authenticated, user]
  );
