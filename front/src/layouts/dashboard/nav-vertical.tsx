...
import { getBasicConfiguration } from 'src/actions/basic-configuration';
...
export function NavVertical({
  ...
  const [smallLogoUrl, setSmallLogoUrl] = useState<string | undefined>();
  ...
  useEffect(() => {
    const loadSmallLogo = async () => {
      try {
        ...
        if (config?.smallLogo) {
          setSmallLogoUrl(config.smallLogo);
        }
      } catch (error) {
        console.error('Error loading small logo:', error);
      }
    };
    ...
    <>
      {slots?.topArea ?? (
        <Box sx={{ display: 'flex', justifyContent: 'center', py: 2.5 }}>
          <Logo smallLogoUrl={smallLogoUrl} />
        </Box>
      )}
