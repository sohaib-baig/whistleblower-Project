...
export function RoleSearch({ redirectPath, sx }: Props) {
  const router = useRouter();

  const [searchQuery, setSearchQuery] = useState('');
  const [selectedItem, setSelectedItem] = useState<IRoleItem | null>(null);
...
function useSearchData(searchQuery: string) {
  const [searchResults, setSearchResults] = useState<IRoleItem[]>([]);
  ...
  const fetchSearchResults = useCallback(async () => {
    ...
    try {
      ...
      const results = _roleItems.filter(({ title }) =>
