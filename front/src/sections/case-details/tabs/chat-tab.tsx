...
import {
  createCaseLog,
  getCaseChatsAuthenticated,
  sendChatAudioAuthenticated,
  sendChatImageAuthenticated,
  sendChatMessageAuthenticated,
  markChatMessagesAsReadAuthenticated,
...
export default function ChatTab({ caseId }: Props) {
  ...
  const fetchChats = useCallback(
    async (isInitialLoad = false) => {
      try {
        ...
        const data = await getCaseChatsAuthenticated(caseId);

        // Convert CaseChatMessage[] to IChatMessage[]
        // Anonymous user messages (created_by === null) appear on LEFT (type: 'system')
        // Case manager/admin/company messages (created_by !== null) appear on RIGHT (type: 'user')
          ...
          return {
            ...
            message:
              chat.type === 'text'
                ? chat.message
                : chat.type === 'audio'
                  ? t('dashboard.case.chatTab.audioMessage')
                  : `${t('dashboard.case.chatTab.imageMessage')}: ${chat.message.split('/').pop()}`,
            sender: senderName,
            timestamp: new Date(chat.timestamp).toLocaleTimeString([], {
              hour: '2-digit',
              minute: '2-digit',
            }),
            // Anonymous users (created_by === null) → LEFT ('system')
            // Authenticated users (case manager, admin, company) → RIGHT ('user')
            type: chat.created_by ? 'user' : 'system',
        ...
        if (isInitialLoad) {
          try {
            await createCaseLog(caseId, 'Tab Viewed', 'Chat Tab (Backend)');
          } catch (logError) {
          ...
          try {
            await markChatMessagesAsReadAuthenticated(caseId);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const sentMessage = await sendChatMessageAuthenticated(caseId, trimmedMessage);

      // Convert to IChatMessage format and add to messages
      // Authenticated users send from backend → RIGHT ('user')
      const senderName =
        sentMessage.creator?.name ||
        sentMessage.sender ||
        t('dashboard.case.chatTab.anonymous', 'Anonymous');
      const newMessageObj: IChatMessage = {
        ...
        type: 'user', // Authenticated users appear on right
  ...
  const handleSendAudioMessage = async (audioBlob: Blob) => {
    try {
      ...
      const sentMessage = await sendChatAudioAuthenticated(caseId, audioBlob);

      // Ensure URL is full path
      const audioUrl = sentMessage.message.startsWith('http')
        ? sentMessage.message
        : `${import.meta.env.VITE_SERVER_URL || ''}${sentMessage.message}`;
      ...
      const newMessageObj: IChatMessage = {
        id: sentMessage.id,
        message: `${t('dashboard.case.chatTab.audioMessage')} (${Math.round(audioBlob.size / 1024)}KB)`,
        sender: senderName,
        timestamp: new Date(sentMessage.timestamp).toLocaleTimeString([], {
          hour: '2-digit',
          minute: '2-digit',
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      const sentMessage = await sendChatImageAuthenticated(caseId, selectedImage);
      ...
      const newMessageObj: IChatMessage = {
        id: sentMessage.id,
        message: `${t('dashboard.case.chatTab.imageMessage')}: ${selectedImage.name}`,
        sender: senderName,
        timestamp: new Date(sentMessage.timestamp).toLocaleTimeString([], {
          hour: '2-digit',
          minute: '2-digit',
            ...
            <Chip
              ...
              label={
                message.messageType === 'audio'
                  ? t('dashboard.case.chatTab.voice')
                  : message.messageType === 'image'
                    ? t('dashboard.case.chatTab.imageMessage')
                    : t('dashboard.case.chatTab.text')
              }
              icon={
                <Iconify
                  icon={
    ...
    <Card sx={{ height: '70vh', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider' }}>
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          <Typography variant="h6">{t('dashboard.case.chatTab.heading')}</Typography>
        </Stack>
      </Box>
        ...
        {loading ? (
          ...
          >
            <Typography variant="body2" color="text.secondary">
              {t('dashboard.case.chatTab.loadingMessages')}
            </Typography>
          </Box>
        ) : messages.length === 0 ? (
          <Box
          ...
          >
            <Typography variant="body2" color="text.secondary">
              {t('dashboard.case.chatTab.noMessages')}
            </Typography>
          </Box>
        ) : (
          <>
      ...
      <Box sx={{ p: 2, borderTop: 1, borderColor: 'divider' }}>
            ...
            <Stack direction="row" spacing={2} alignItems="center">
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Typography variant="body2" color="error.main">
                  {t('dashboard.case.chatTab.recording')} {formatRecordingTime(recordingTime)}
                </Typography>
              </Box>
          ...
          <Box sx={{ mb: 2, p: 2, border: 1, borderColor: 'divider', borderRadius: 1 }}>
            <Stack direction="row" spacing={2} alignItems="center">
              ...
              <Stack direction="row" spacing={1}>
                ...
                >
                  {t('dashboard.case.chatTab.cancel')}
                </Button>
              </Stack>
            </Stack>
          </Box>
        ...
        <Stack direction="row" spacing={1}>
          <TextField
            fullWidth
            placeholder={t('dashboard.case.chatTab.typeMessagePlaceholder')}
            value={newMessage}
          ...
          {/* Image Upload Button */}
          <Tooltip title={t('dashboard.case.chatTab.attachImage')}>
          ...
          <Tooltip
            title={
              isRecording
                ? t('dashboard.case.chatTab.stopRecording')
                : t('dashboard.case.chatTab.startRecording')
            }
          >
            <IconButton
          ...
          {/* Send Button */}
          <Tooltip title={t('dashboard.case.chatTab.sendMessage')}>
        ...
        {/* Upload Progress - Removed unused isUploading state */}
      </Box>
    </Card>
  );
}
