...
export function IntegrationSearch({ redirectPath, sx }: Props) {
  const router = useRouter();

  const [searchQuery, setSearchQuery] = useState('');
  const [selectedItem, setSelectedItem] = useState<IIntegrationItem | null>(null);

  const debouncedQuery = useDebounce(searchQuery);
  const { searchResults: options, searchLoading: loading } = useSearchData(debouncedQuery);

  const handleChange = useCallback(
    (item: IIntegrationItem | null) => {
...
function useSearchData(searchQuery: string) {
  const [searchResults, setSearchResults] = useState<IIntegrationItem[]>([]);
  ...
  const fetchSearchResults = useCallback(async () => {
    ...
    try {
      ...
      const results = _integrationItems.filter(({ name, endpoint }) =>
        [name, endpoint].some((field) => field?.toLowerCase().includes(searchQuery.toLowerCase()))
      );
