...
export const _roleItems = Array.from({ length: 2 }, (_, index) => {
  ...
  const experience =
    ROLE_EXPERIENCE_OPTIONS.map((option) => option.label)[index] ||
    ROLE_EXPERIENCE_OPTIONS[1].label;
  ...
  const permissions = ROLE_MODULES.map((module) => {
    const modulePermissions =
      module.id === 'dashboard'
        ? [ROLE_PERMISSIONS[0]] // Dashboard only has List
        : ROLE_PERMISSIONS.slice(0, 4); // Others have List, Create, Update, Delete
    ...
    return {
      ...
      permissions: modulePermissions.map((permission) => ({
        ...
        checked: index === 0 ? true : Math.random() > 0.5, // Admin has all checked, Company has random
      })),
    };
  });
